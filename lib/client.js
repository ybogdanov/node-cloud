...
Client.prototype._loadConfig = function(config) {
    ...
    Object.keys(config.actions).forEach(function(actionName){
        if (typeof config.actions[actionName] !== 'object') {
            return self.emit('error', 'Configuration error: action should be object - ' + actionName);
        }
        self._actions[actionName] = config.actions[actionName];
        
        // Check for collision
        if (self[actionName]) {
            return self.emit('error', 'Configuration collosion: reserved action name ' + actionName);
        }
        ...
        self[actionName] = function action() {
            ...
            self._call(actionName, args, callback);
        }
    })
}
