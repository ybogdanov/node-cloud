...
Worker.prototype._loadConfig = function(config) {
    ...
    if (typeof config.redis !== 'undefined') {
        utils.extend(config.redis, Worker.defaults.redis);
    }

    utils.extend(config, Worker.defaults);
...
Worker.prototype.getRedisClient = function(type) {
    ...
    if (!this.redisClients[type]) {
        ...
        if (this.config.redis.db) {
            this.redisClients[type].select(this.config.redis.db);
        }
    }
...
Worker.prototype._pull = function() {
    ...
    if (args.length) {
        ...
        self._debug('pulling %s', args.join(', '), timeout);
    ...
    if (alive) {
        ...
        Object.keys(this._actions).forEach(function(key){
            ...
            info.actions[key] = {
                ...
                tasksDone : action.tasksDone,
                avgTime : action.avgTime
            }
        })
        
        redis.setex(key, Math.ceil(this.config.maintainTimeout / 1000), JSON.stringify(info), function(err){
            if (err) return self.emit('error', err);
        });
...
Worker.prototype._debug = function() {
    if (this.config.debug) {
        arguments[0] = '[/' + this.ns + '/' + this.name + ' debug] ' + arguments[0];
